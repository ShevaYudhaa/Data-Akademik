...
function Jurusan() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!kode || !nama) return alert("Isi semua data!");
        let updatedData;
        if (editIndex !== null) {
        updatedData = updateItem(STORAGE_KEY, editIndex, { kode, nama });
        setEditIndex(null);
        } else {
        updatedData = addItem(STORAGE_KEY, { kode, nama });
        }
        setData(updatedData);
        setKode(""); setNama("");
    };
    ...
    const handleEdit = (i) => {
        const item = data[i];
        setKode(item.kode); setNama(item.nama); setEditIndex(i);
    };
    ...
    return (
        ...
        <h2 className="text-primary fw-bold mb-3">Master Data Jurusan</h2>
        <form onSubmit={handleSubmit} className="mb-4 bg-light p-3 rounded shadow-sm">
            <FormInput label="Kode Jurusan" value={kode} onChange={(e) => setKode(e.target.value)} required />
            <FormInput label="Nama Jurusan" value={nama} onChange={(e) => setNama(e.target.value)} required />
            <button type="submit" className={`btn w-100 ${editIndex !== null ? "btn-success" : "btn-primary"}`}>
            {editIndex !== null ? "Update Data" : "Tambah Jurusan"}
            </button>
        </form>
